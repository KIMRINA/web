...
public class MemberDAO {
	...
	public MemberVO selectOne(MemberVO memberVO) {
		...
		try {
			...
			String sql = "SELECT DEPARTMENT_ID, DEPARTMENT_NAME, MANAGER_ID mgr_id, LOCATION_ID" + " FROM DEPARTMENTS"
					+ " WHERE DEPARTMENT_ID=?"; // 컨+쉬+x 대문자, 컨+쉬+y 소문자 변환가능. 쿼리 엔터해서 쓸거면 앞에 공백 붙이기
			pstmt = conn.prepareStatement(sql); // 미리 sql 구문이 준비가 되어야한다
			pstmt.setInt(1, memberVO.getDepartment_id()); // ?의 첫번째 자리에 올 값 지정
	...
	// 전체 조회
	public ArrayList<MemberVO> selectAll() { // 조회가 여러건이면 DeptVO를 list에 담음
		MemberVO resultVO = null; // select할때는 리턴값이 필요해서 리턴값을 저장할 변수 선언
		ArrayList<MemberVO> list = new ArrayList<MemberVO>(); // 결과값을 저장할 list 변수 객체 선언
		...
		try {
			...
			while (rs.next()) { // 여러건 조회라서 while를 사용. next()로 한건 한건마다 true 인지 false인지 확인하고 이동함
				resultVO = new MemberVO(); // 레코드 한건을 resultVO에 담음
				resultVO.setDepartment_id(rs.getInt(1)); // 컬럼이 첫번째 자리라서 1을 입력한거임
				resultVO.setDepartment_name(rs.getString("department_name"));
				resultVO.setManager_id(rs.getInt("mgr_id")); // 컬럼명에다가 별칭있으면 별칭을 넣어줘야함
				resultVO.setLocation_id(rs.getInt("LOCATION_ID")); // 대소문자 구별 없음
	...
	public void update(MemberVO memberVO) {
		try {
			...
			pstmt = conn.prepareStatement(sql); // 미리 sql 구문이 준비가 되어야한다
			pstmt.setString(1, memberVO.getDepartment_name()); // ?의 첫번째 자리에 올 값 지정
			pstmt.setInt(2, memberVO.getDepartment_id()); // ?의 두번째 자리에 올 값 지정
	...
	public void insert(MemberVO memberVO) {
		try {
			...
			String sql = "insert into departments (department_id, department_name)" + "values("
					+ memberVO.getDepartment_id() + ",'" + memberVO.getDepartment_name() + "')";
